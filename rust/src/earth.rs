//! Earth related calculations
use crate::{jd, util};

const NUTATION_PERTURBATION_TERMS: [(i8, i8, i8, i8, i8, i64, f64); 63] = [
    (0, 0, 0, 0, 1, -171996, -174.2),
    (-2, 0, 0, 2, 2, -13187, -1.6),
    (0, 0, 0, 2, 2, -2274, -0.2),
    (0, 0, 0, 0, 2, 2062, 0.2),
    (0, 1, 0, 0, 0, 1426, -3.4),
    (0, 0, 1, 0, 0, 712, 0.1),
    (-2, 1, 0, 2, 2, -517, 1.2),
    (0, 0, 0, 2, 1, -386, -0.4),
    (0, 0, 1, 2, 2, -301, 0.0),
    (-2, -1, 0, 2, 2, 217, -0.5),
    (-2, 0, 1, 0, 0, -158, 0.0),
    (-2, 0, 0, 2, 1, 129, 0.1),
    (0, 0, -1, 2, 2, 123, 0.0),
    (2, 0, 0, 0, 0, 63, 0.0),
    (0, 0, 1, 0, 1, 63, 0.1),
    (2, 0, -1, 2, 2, -59, 0.0),
    (0, 0, -1, 0, 1, -58, -0.1),
    (0, 0, 1, 2, 1, -51, 0.0),
    (-2, 0, 2, 0, 0, 48, 0.0),
    (0, 0, -2, 2, 1, 46, 0.0),
    (2, 0, 0, 2, 2, -38, 0.0),
    (0, 0, 2, 2, 2, -31, 0.0),
    (0, 0, 2, 0, 0, 29, 0.0),
    (-2, 0, 1, 2, 2, 29, 0.0),
    (0, 0, 0, 2, 0, 26, 0.0),
    (-2, 0, 0, 2, 0, -22, 0.0),
    (0, 0, -1, 2, 1, 21, 0.0),
    (0, 2, 0, 0, 0, 17, -0.1),
    (2, 0, -1, 0, 1, 16, 0.0),
    (-2, 2, 0, 2, 2, -16, 0.1),
    (0, 1, 0, 0, 1, -15, 0.0),
    (-2, 0, 1, 0, 1, -13, 0.0),
    (0, -1, 0, 0, 1, -12, 0.0),
    (0, 0, 2, -2, 0, 11, 0.0),
    (2, 0, -1, 2, 1, -10, 0.0),
    (2, 0, 1, 2, 2, -8, 0.0),
    (0, 1, 0, 2, 2, 7, 0.0),
    (-2, 1, 1, 0, 0, -7, 0.0),
    (0, -1, 0, 2, 2, -7, 0.0),
    (2, 0, 0, 2, 1, -7, 0.0),
    (2, 0, 1, 0, 0, 6, 0.0),
    (-2, 0, 2, 2, 2, 6, 0.0),
    (-2, 0, 1, 2, 1, 6, 0.0),
    (2, 0, -2, 0, 1, -6, 0.0),
    (2, 0, 0, 0, 1, -6, 0.0),
    (0, -1, 1, 0, 0, 5, 0.0),
    (-2, -1, 0, 2, 1, -5, 0.0),
    (-2, 0, 0, 0, 1, -5, 0.0),
    (0, 0, 2, 2, 1, -5, 0.0),
    (-2, 0, 2, 0, 1, 4, 0.0),
    (-2, 1, 0, 2, 1, 4, 0.0),
    (0, 0, 1, -2, 0, 4, 0.0),
    (-1, 0, 1, 0, 0, -4, 0.0),
    (-2, 1, 0, 0, 0, -4, 0.0),
    (1, 0, 0, 0, 0, -4, 0.0),
    (0, 0, 1, 2, 0, 3, 0.0),
    (0, 0, -2, 2, 2, -3, 0.0),
    (-1, -1, 1, 0, 0, -3, 0.0),
    (0, 1, 1, 0, 0, -3, 0.0),
    (0, -1, 1, 2, 2, -3, 0.0),
    (2, -1, -1, 2, 2, -3, 0.0),
    (0, 0, 3, 2, 2, -3, 0.0),
    (2, -1, 0, 2, 2, -3, 0.0),
];

/// Calculate Earth's eccentricity, eq (47.6).
/// In: Julian day in dynamical time
pub fn eccentricity(jd: f64) -> f64 {
    let t = jd::from_epoch_j2000(jd);
    let t2 = t * t;

    1.0 - 0.002516 * t - 0.0000074 * t2
}

/// Nutation of the Earth, Meeus chapter 22
/// In: Julian day in dynamical time
/// Out: correction term in degrees
pub fn nutation(jd: f64) -> f64 {
    let t = jd::from_epoch_j2000(jd);
    let t2 = t * t;
    let t3 = t * t2;

    let d = 297.85036 + (445267.111480 * t) - (0.0019142 * t2) + (t3 / 189_474.0);
    let d = util::map_to_0_to_360(d);

    let m = 357.52772 + (35_999.050340 * t) - (0.0001603 * t2) - (t3 / 300_000.0);
    let m = util::map_to_0_to_360(m);

    let m_prime = 134.96298 + (477_198.867398 * t) + (0.0086972 * t2) + (t3 / 56_250.0);
    let m_prime = util::map_to_0_to_360(m_prime);

    let f = 93.27191 + (483_202.017538 * t) - (0.0036825 * t2) + (t3 / 327_270.0);
    let f = util::map_to_0_to_360(f);

    let omega = 125.04452 - (1934.136261 * t) + (0.0020708 * t2) + (t3 / 450_000.0);
    let omega = util::map_to_0_to_360(omega);

    let delta_psi = NUTATION_PERTURBATION_TERMS.iter().fold(0.0, |accum, &c| {
        let sin_arg = c.0 as f64 * d
            + c.1 as f64 * m
            + c.2 as f64 * m_prime
            + c.3 as f64 * f
            + c.4 as f64 * omega;
        let sin_arg = util::to_radians(sin_arg);
        let value = (c.5 as f64 + c.6 * t) * sin_arg.sin() * 0.0001;
        accum + value
    });

    delta_psi
}

#[cfg(test)]
mod tests {
    use super::*;
    use assert_approx_eq::assert_approx_eq;

    #[test]
    fn eccentricity_test() {
        // Arrange

        // SS: April 12th, 1992, 0h TD
        let jd = 2_448_724.5;

        // Act
        let e = eccentricity(jd);

        // Assert
        assert_approx_eq!(1.000194, e, 0.000001)
    }

    #[test]
    fn nutation_test_1() {
        // Arrange
        let jd = jd::from_date(1987, 4, 10, 0.0);

        // Act
        let delta_psi = nutation(jd);

        // Assert
        assert_approx_eq!(-3.788, delta_psi, 0.001)
    }

    #[test]
    fn nutation_test_2() {
        // Arrange

        // SS: 1992 April 12, 0h TD
        let jd = jd::from_date(1992, 4, 12, 0.0);

        // Act
        let delta_psi = util::arcsec_to_degrees(nutation(jd));

        // Assert
        assert_approx_eq!(0.00461, delta_psi, 0.001)
    }
}
